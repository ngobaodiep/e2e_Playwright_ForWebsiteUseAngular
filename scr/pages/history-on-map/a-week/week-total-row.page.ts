...
export default class WeekTotalRowPage extends Wrapper{
    get itself(): Locator {
        return this._itself;
    }
    ...
    private async getElements(itself : Locator){
        this.itself = itself;
        this.distance = await this.itself.locator(el.journey.week_total.distance);
        this.stopped_time = await this.itself.locator(el.journey.week_total.stopped_time);
        this.engagement_time = await this.itself.locator(el.journey.week_total.engagement_time);
        this.duration = await this.itself.locator(el.journey.week_total.duration);
    }
}